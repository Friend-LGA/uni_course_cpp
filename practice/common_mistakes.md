...
`constexpr` в свою очередь, говорит компилятору, что данную переменную (или функцию) можно рассчитать на этапе компиляции, и вместо самой переменной, подставить в код непосредственно её значение.
...
В продолжение разговора о `Single-Responsibility`. Этот принцип так же относится и к функциям.

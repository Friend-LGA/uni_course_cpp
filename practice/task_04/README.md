...
Обратите вниманием что в `C++` используется `enum class`, тогда как в `C` - просто `enum`.
`enum class` лучше, так как он позволяет избежать автоматического приведения типов, что делает код надежней и позволяет компилятору отловить ошибки на ранней стадии.
